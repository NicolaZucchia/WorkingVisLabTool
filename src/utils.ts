...
function getFilteredIndices(size: number, minPredictionDifference: number, model1BrushedExtent: [number, number], model2BrushedExtent: [number, number], minDiffFromTruth1: number, minDiffFromTruth2: number, gt: number[], predictions: [number[], number[]]): number[] {
    return range(size).filter(i => {
        return Math.abs(predictions[0][i] - predictions[1][i]) >= minPredictionDifference
        && Math.abs(predictions[0][i] - gt[i]) >= minDiffFromTruth1 && Math.abs(predictions[1][i] - gt[i]) >= minDiffFromTruth2
